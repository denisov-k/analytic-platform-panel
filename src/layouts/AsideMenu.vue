<template>
    <ul>
        <li v-for="menuItem in menuItems" :key="menuItem.path">
            <router-link :to="menuItem.name ? { name: menuItem.name } : menuItem.path" :exact-active-class="menuExactActiveClass(menuItem)" active-class="active" exact>{{menuItem.meta.title}}</router-link>
...
<style>
    ...
    .uk-nav li a:hover{ border-bottom: 0px; color: #fff;}
    ...
    .uk-nav li a:hover:after{ width: 100%; }
